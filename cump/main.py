...
@click.group()
@click.pass_context
@click.option('--credentials', '-c', default="~/.cump/settings.ini", type=click.Path(exists=False, file_okay=True, readable=True, dir_okay=False), help='credentials.ini file path')
...
def cli(ctx, credentials, s3_access_key, s3_public_key, s3_bucket, http_auth, http_host, project, verbose):
    click.echo(click.style('ATTENTION', fg='red') + ' is workd!')
